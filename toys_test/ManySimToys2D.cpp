...
void GenerateToys(std::string const &outputDir, int nToys, bool toPlot) {
  ...
  for (int i = 0; i < nToys; i++) {
    ...
    RooRealVar a0SigmaBuSignalPi(
        ("a0SigmaBuSignalPi_" + std::to_string(i)).c_str(), "", 3.3762e+02);//, 300, 400);
    RooRealVar a1SigmaBuSignalPi(
        ("a1SigmaBuSignalPi_" + std::to_string(i)).c_str(), "", -4.1666e+00);//, -10, 10);
    RooRealVar a2SigmaBuSignalPi(
        ("a2SigmaBuSignalPi_" + std::to_string(i)).c_str(), "", 1.4773e-02);//, -0.1, 0.1);
    ...
    RooRealVar sigma21FracBuSignal(
        ("sigma21FracBuSignal_" + std::to_string(i)).c_str(), "", 3.8961e-01);//, -1, 1);
    ...
    RooRealVar fracPdf1BuSignal(
        ("fracPdf1BuSignal_" + std::to_string(i)).c_str(), "", 5.6000e-02);
