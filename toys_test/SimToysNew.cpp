...
void PlotComponent(Neutral neutral, Bachelor bachelor, Variable variable,
  ...
  switch (variable) {
    case Variable::bu:
      title = "m[D^{*0}" + EnumToLabel(bachelor) + "] - m[D^{*0}]";
      break;
    case Variable::delta:
      switch (neutral) {
        case Neutral::gamma:
...
void GenerateToys(std::string const &outputDir, int nToys, bool toPlot) {
  ...
  // ---------------------------- Fitting Vars ----------------------------
  RooRealVar buMass("Bu_Delta_M", "m[D^{*0}#pi^{#pm}] - m[D^{*0}]", bu_low, bu_high, "MeV/c^{2}");
  RooRealVar deltaMass("Delta_M", "m[D^{*0}] - m[D^{0}]", delta_low, delta_high, "MeV/c^{2}");
  ...
  for (int i = 0; i < nToys; i++) {
    ...
    RooRealVar a0SigmaBuSignalPi(
        ("a0SigmaBuSignalPi_" + std::to_string(i)).c_str(), "", 2.9812e+02);//, 300, 400);
    RooRealVar a1SigmaBuSignalPi(
        ("a1SigmaBuSignalPi_" + std::to_string(i)).c_str(), "", -3.5731e+00);//, -10, 10);
    RooRealVar a2SigmaBuSignalPi(
        ("a2SigmaBuSignalPi_" + std::to_string(i)).c_str(), "", 1.2657e-02);//, -0.1, 0.1);
    ...
    RooRealVar sigma21FracBuSignal(
        ("sigma21FracBuSignal_" + std::to_string(i)).c_str(), "", 4.0662e-01);//, -1, 1);
    ...
    RooRealVar fracPdf1BuSignal(
        ("fracPdf1BuSignal_" + std::to_string(i)).c_str(), "", 6.0648e-02);
