...
Model::Model(Configuration &config, int _uniqueId)
    : uniqueId(_uniqueId),
      ...
      // -0.00001), buN2(("buN2_" + std::to_string(uniqueId)).c_str(), "", 10),
      ...
      // -0.00001), deltaN2(("deltaN2_" + std::to_string(uniqueId)).c_str(), "",
      ...
      // "", 5.2445e+03, 5235, 5255), buBkgSigma(("buBkgSigma_" +
      // std::to_string(uniqueId)).c_str(), "", 8.5073e+01, 75, 95), m(B): 5170
      // - 5370
      buBkgMean(("buBkgMean_" + std::to_string(uniqueId)).c_str(), "",
                5.2466e+03),
  ...
  pdf2D = std::unique_ptr<RooAddPdf>(new RooAddPdf(
      ("pdf2D_" + std::to_string(uniqueId)).c_str(), "", functions2D, yields2D));
}
