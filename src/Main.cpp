// ASK JOJ: HOW TO USE ENUMS WHEN I AM GETTING SUBSTRINGS
        ...
        for (unsigned int dCounter = 0; dCounter < daughtersVec.size();
          ...
          for (unsigned int cCounter = 0; cCounter < chargeVec.size();
            ...
            if (!fexists(dsFile)) {
              ...
              TFile in(dsFile.c_str());
              if (chargeVec[cCounter] == Charge::plus) {
                RooDataSet *inputDataSet =
                    dynamic_cast<RooDataSet *>(in.FindObject("inputDataSet"));
                ...
                RooDataSet *minusDataSet =
                    dynamic_cast<RooDataSet *>(in.FindObject("minusDataSet"));
                if (minusDataSet == nullptr) {
                  throw std::runtime_error("Data set does not exist.");
                } else {
                  std::cout << "minusDataSet extracted... \n";
                  fullDataSet.append(*minusDataSet);
                  std::cout << "Appended to full data set...\n";
                }
              }
            }
