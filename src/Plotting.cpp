...
void LaTeXYields(Configuration &config, std::vector<PdfBase *> &pdfs,
  ...
  for (auto &p : pdfs) {
    ...
    outfile << " & ";
    outfile << "$"
            << to_string_with_precision(p->orEffBu2Dst0h_D0pi0_WN().getVal(), 3)
            << "$ &";
    outfile << "$"
            << to_string_with_precision(
                   p->N_trueId_Bu2Dst0h_D0pi0_WN().getVal(), 0)
            << " \\pm "
            << to_string_with_precision(
                   p->N_trueId_Bu2Dst0h_D0pi0_WN().getPropagatedError(*result),
                   0)
            << "$ &";
    outfile << "$"
            << to_string_with_precision(p->buEffBu2Dst0h_D0pi0_WN().getVal(), 3)
            << "$ &";
    outfile << std::fixed << "$"
            << to_string_with_precision(
            ...
            << "$ &";
    outfile << "$"
            << to_string_with_precision(p->deltaEffBu2Dst0h_D0pi0_WN().getVal(),
                                        3)
            << "$ &";
    outfile << "$"
            << to_string_with_precision(
    ...
    outfile << " & ";
    outfile << "$"
            << to_string_with_precision(p->orEffBu2Dst0h_D0gamma_WN().getVal(),
                                        3)
            << "$ &";
    outfile
        << "$"
        << to_string_with_precision(p->N_trueId_Bu2Dst0h_D0gamma_WN().getVal(),
                                    0)
        << " \\pm "
        << to_string_with_precision(
               p->N_trueId_Bu2Dst0h_D0gamma_WN().getPropagatedError(*result), 0)
        << "$ &";
    outfile << "$"
            << to_string_with_precision(p->buEffBu2Dst0h_D0gamma_WN().getVal(),
                                        3)
            << "$ &";
    outfile << std::fixed << "$"
            << to_string_with_precision(
    ...
    outfile << " & ";
    outfile << "$"
            << to_string_with_precision(p->orEffBu2Dst0hst_D0pi0().getVal(), 3)
            << "$ &";
    outfile << "$"
            << to_string_with_precision(p->N_trueId_Bu2Dst0hst_D0pi0().getVal(),
                                        0)
            << " \\pm "
            << to_string_with_precision(
                   p->N_trueId_Bu2Dst0hst_D0pi0().getPropagatedError(*result),
                   0)
            << "$ &";
    outfile << "$"
            << to_string_with_precision(p->buEffBu2Dst0hst_D0pi0().getVal(), 3)
            << "$ &";
    outfile
        << std::fixed << "$"
        << to_string_with_precision(p->N_trueId_Bu_Bu2Dst0hst_D0pi0().getVal(),
                                    0)
        << " \\pm "
        << to_string_with_precision(
               p->N_trueId_Bu_Bu2Dst0hst_D0pi0().getPropagatedError(*result), 0)
        << "$ &";
    outfile << "$"
            << to_string_with_precision(p->deltaEffBu2Dst0hst_D0pi0().getVal(),
                                        3)
            << "$ &";
    outfile << "$"
            << to_string_with_precision(
                   p->N_trueId_Delta_Bu2Dst0hst_D0pi0().getVal(), 0)
            << " \\pm "
            << to_string_with_precision(
                   p->N_trueId_Delta_Bu2Dst0hst_D0pi0().getPropagatedError(
                       *result),
                   0)
            << "$ ";
    if (n == Neutral::gamma) {
      outfile << "& $"
              << to_string_with_precision(
                     p->buPartialEffBu2Dst0hst_D0pi0().getVal(), 3)
              << "$ &";
      outfile << "$"
              << to_string_with_precision(
                     p->N_trueId_BuPartial_Bu2Dst0hst_D0pi0().getVal(), 0)
              << " \\pm "
              << to_string_with_precision(
                     p->N_trueId_BuPartial_Bu2Dst0hst_D0pi0()
                         .getPropagatedError(*result),
                     0)
              << "$";
    }
    ...
    outfile << " & ";
    outfile << "$"
            << to_string_with_precision(p->orEffBu2Dst0hst_D0gamma().getVal(),
                                        3)
            << "$ &";
    outfile << "$"
            << to_string_with_precision(
                   p->N_trueId_Bu2Dst0hst_D0gamma().getVal(), 0)
            << " \\pm "
            << to_string_with_precision(
                   p->N_trueId_Bu2Dst0hst_D0gamma().getPropagatedError(*result),
                   0)
            << "$ &";
    outfile << "$"
            << to_string_with_precision(p->buEffBu2Dst0hst_D0gamma().getVal(),
                                        3)
            << "$ &";
    outfile << std::fixed << "$"
            << to_string_with_precision(
                   p->N_trueId_Bu_Bu2Dst0hst_D0gamma().getVal(), 0)
            << " \\pm "
            << to_string_with_precision(
                   p->N_trueId_Bu_Bu2Dst0hst_D0gamma().getPropagatedError(
                       *result),
                   0)
            << "$ &";
    outfile << "$"
            << to_string_with_precision(
                   p->deltaEffBu2Dst0hst_D0gamma().getVal(), 3)
            << "$ &";
    outfile << "$"
            << to_string_with_precision(
                   p->N_trueId_Delta_Bu2Dst0hst_D0gamma().getVal(), 0)
            << " \\pm "
            << to_string_with_precision(
                   p->N_trueId_Delta_Bu2Dst0hst_D0gamma().getPropagatedError(
                       *result),
                   0)
            << "$ ";
    if (n == Neutral::gamma) {
      outfile << "& $"
              << to_string_with_precision(
                     p->buPartialEffBu2Dst0hst_D0gamma().getVal(), 3)
              << "$ &";
      outfile << "$"
              << to_string_with_precision(
                     p->N_trueId_BuPartial_Bu2Dst0hst_D0gamma().getVal(), 0)
              << " \\pm "
              << to_string_with_precision(
                     p->N_trueId_BuPartial_Bu2Dst0hst_D0gamma()
                         .getPropagatedError(*result),
                     0)
              << "$";
    }
...
void Plotting1D(int const id, PdfBase &pdf, Configuration &config,
  ...
  if (bachelor == Bachelor::pi) {
    ...
    if (neutral == Neutral::gamma) {
      labels.AddEntry(
          hist_Bu2Dst0rho_D0gamma.get(),
          ReturnLaTeXLabel(Mode::Bu2Dst0rho_D0gamma, daughters, charge, false)
              .c_str(),
          "l");
    }
    labels.AddEntry(
    ...
    if (neutral == Neutral::gamma) {
      labels.AddEntry(
          hist_Bu2Dst0Kst_D0gamma.get(),
          ReturnLaTeXLabel(Mode::Bu2Dst0Kst_D0gamma, daughters, charge, false)
              .c_str(),
          "l");
    }
    labels.AddEntry(
  ...
  void Plotting2D(RooDataSet & dataSet, int const id, PdfBase &pdf,
    ...
    auto buDeltaAbsData = dataSet.reduce(
        ("fitting==fitting::" + ComposeFittingName(Mass::buDelta, neutral,
    ...
    dataHist2d->SetTitle(("B^{" + EnumToLabel(charge) +
                          "}#rightarrow#font[132]{[}#font[132]{[}" +
                          EnumToLabel(daughters, charge) +
    ...
    if (config.noFit() == false) {
      ...
      if (mass == Mass::buDelta) {
        if (neutral == Neutral::pi0) {
          ...
          if (bachelor == Bachelor::k) {
            ...
            pdfCharVec.emplace_back(
                pdf.pdfBu_misId_Bu2Dst0hst_D0pi0().GetName());
          }
        } else {
        ...
        pdfCharVec.emplace_back(
            pdf.pdfBuPartial_Bu2Dst0hst_D0gamma().GetName());
      ...
      while (pdfCharVec.size() > 0) {
        ...
        simPdf.plotOn(
            ...
            RooFit::FillColor(
                colorMap[EnumToString(bachelor)][modeStr.c_str()]),
