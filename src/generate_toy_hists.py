...
u_dict = {'bu': {'kernel': [], 'data': []}, 'delta': {'kernel': [], 'data': []}}
norm_dict = {'bu': {'kernel': [], 'data': []}, 'delta': {'kernel': [], 'data': []}}
...
for mass, d in u_dict.items():
  for pdf, u in d.items():
    ...
    norm_dict[mass][pdf] = np.divide(
        u,
        hist_dict[mass][pdf].GetEntries() * np.ones(limits_dict[mass]['bins']))
    fig = plt.plot()
    ...
    plt.savefig('kde_plots/' + mass + '_' + pdf + '.png')
    plt.clf()
  fig = plt.plot()
  plt.errorbar(xaxis_dict[mass],
