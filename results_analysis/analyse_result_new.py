          ...
          for par in comb_result.floatParsFinal():
            ...
            if par_name in fit_result:
              if par_name[:5] != 'N_tot' and par_name[:2] != 'BR':
                par_comb = par.getVal()
                par_comb_err = par.getError()
                par_og = fit_result[par_name]['Value']
                par_err = fit_result[par_name]['Statistical Error']
                par_shift = par_comb - par_og
                # print(f'{par_name}:\t{par_og} +/- {par_err}')
                # print(f'w/ combinatorial:\t\t\t{par_comb} +/- {par_comb_err}')
                # print(f'Systematic error (%):\t\t\t{(par_shift/par_err)*100}\n')
                if par_name in syst_comb_dict:
                  syst_comb_dict[par_name] = par_shift
                else:
                  syst_comb_dict[par_name] = par_shift
        else:
